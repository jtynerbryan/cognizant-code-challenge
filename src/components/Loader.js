      ...
      this.state.cities.forEach(city => {
        ...
        .then(res => {
          try {
            ...
            this.setState({ ids: [...this.state.ids, 'invalid city']})
          }
        })
      })
    } else if (this.state.cities.length === this.state.ids.length && this.state.weatherData.length === 0) {
        if (this.state.ids.includes('invalid city')) {
          alert('Sorry, one of the cities you entered was not found in the database. Please try again')
